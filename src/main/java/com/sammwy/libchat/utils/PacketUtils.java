package com.sammwy.libchat.utils;
            ...
            for (Constructor<?> constructor : nmschatpacket.getConstructors()) {
                ...
                if (types.length == 2) {
                    if (types[1] == byte.class) {
                        packet = nmschatpacket.getConstructor(nmschatcomponent, byte.class).newInstance(chatcomponent,
                                type);
                    } else {
                        Class<?> nmschatmessagetype = ReflectionUtils.getNMSClass("ChatMessageType");
                        packet = nmschatpacket.getConstructor(nmschatcomponent, nmschatmessagetype).newInstance(
                                chatcomponent, Array.get(nmschatmessagetype.getMethod("values").invoke(null), type));
                    }
                }
            }
